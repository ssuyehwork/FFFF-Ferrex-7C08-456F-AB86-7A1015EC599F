...
impl<'a> PathResolver<'a> {
    pub fn new(frns: &'a [u64], parent_frns: &'a [u64], name_offsets: &'a [u32], string_pool: &'a [u8]) -> Self {
        let mut frn_to_idx = HashMap::with_capacity(frns.len());
    ...
    pub fn resolve(&self, frn: u64) -> String {
        ...
        while current_frn != 5 && current_frn != 0 {
            if let Some(&idx) = self.frn_to_idx.get(&current_frn) {
                let name = get_name_from_pool(self.string_pool, self.name_offsets[idx] as usize);
                parts.push(name);
                current_frn = self.parent_frns[idx];
            } else {
...
impl<'a> Searcher<'a> {
    pub fn new(frns: &'a [u64], name_offsets: &'a [u32], string_pool: &'a [u8], sorted_idx: &'a [u32]) -> Self {
        Self { frns, name_offsets, string_pool, sorted_idx }
    }
    ...
    pub fn search_with_ext(&self, query: &str, ext: Option<&str>) -> Vec<usize> {
        ...
        let start_range = if !query_bytes.is_empty() {
            self.sorted_idx.partition_point(|&idx| {
                let name = get_name_from_pool(self.string_pool, self.name_offsets[idx as usize] as usize);
        ...
        self.sorted_idx[start_range..].into_par_iter()
            .filter_map(|&idx_u32| {
                ...
                if let Some(ref e) = ext_lower {
                    if !name_lower.ends_with(e) { return None; }
                }

                if let Some(ref f) = finder {
                    if f.find(name_lower.as_bytes()).is_none() { return None; }
                }
